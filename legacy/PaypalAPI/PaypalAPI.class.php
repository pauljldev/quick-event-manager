	...
	class PaypalAPI {
		...
		public function setAttribute( $attribute, $value ) {
			...
			if ( ! empty( $this->method ) ) {
				...
				if ( array_key_exists( $attribute, $this->fields[ $this->method ] ) ) {
					...
					if ( $et == gettype( $value ) || ( $et == 'float' && is_numeric( $value ) ) || ( $et == 'int' && in_array( gettype( $value ), array(
								'int',
								'integer'
							) ) ) ) {
		...
		public function execute() {
			...
			*/
			...
			$this->response = $http->return_body;
					...
					switch ( $type ) {
						case 'L_PAYMENTREQUEST':
							...
							*/
							...
							if ( ! isset( $items[ $order ][ $item ] ) ) {
								$items[ $order ][ $item ] = array();
							}
